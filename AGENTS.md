...
>
> **This is a non-negotiable, hard requirement.** Before generating, editing, or suggesting ANY code in this repository, you MUST first open and read every relevant instructions file inside the [`/docs`](./docs) directory that relates to the area you are touching (auth, UI components, database, routing, etc.).
>
> - Do NOT skip this step, even for "small" or "obvious" changes.
> - Do NOT rely on memory of a previous read within the same session — re-check the file if there is any doubt about its current contents.
> - If a new file is added to `/docs`, treat it as required reading before touching the related area.
> - If you are unsure whether a `/docs` file is relevant, open it and check rather than assuming it isn't.
> - Only after reading the applicable `/docs` file(s) should you proceed to write or modify code.
>
> Failure to read the relevant `/docs` file(s) first is considered a critical process violation, regardless of whether the resulting code happens to be correct.
...
For detailed guidelines on specific topics, refer to the modular documentation in the `/docs` directory. **ALWAYS read the relevant .md file(s) in full BEFORE generating, editing, or suggesting any code** — this is required every time, not just the first time a topic comes up:

- [Authentication](./docs/auth.md) — Clerk-only auth, protected routes, modal sign-in/sign-up. **Read this before touching any auth, sign-in/sign-up, session, or protected-route code.**
- [UI Components](./docs/ui-components.md) — shadcn/ui only, no custom components. **Read this before creating or modifying any component in `components/` or any UI in `app/`.**

If work spans multiple areas (e.g., an authenticated page with UI components), read ALL applicable docs files first, not just one.
